...
export default function ReviewOrganizations() {
  ...
  const fetchPendingOrganizations = async () => {
    try {
  const response = await fetch('https://aid-link-11.onrender.com/api/admin/pending-organizations');
      if (response.ok) {
  ...
  const approveOrg = async (id) => {
    try {
  const response = await fetch(`https://aid-link-11.onrender.com/api/admin/approve-organization/${id}`, { 
        method: 'POST' 
      });
      if (response.ok) {
  ...
  console.log('Making request to:', `https://aid-link-11.onrender.com/api/admin/reject-organization/${id}`);

    try {
  const response = await fetch(`https://aid-link-11.onrender.com/api/admin/reject-organization/${id}`, { 
        method: 'POST',
        headers: {
